...
def load_model(filename, model_dir=None, map_location=None):
...
def densenet_121(pretrained=False, **kwargs):
    ...
    model = DenseNetEfficient(num_init_features=64, growth_rate=32, block_config=(6, 12, 24, 16),
                     **kwargs)
    if pretrained:
        model.load_state_dict(load_model('densenet-121.pth'))
    return model


def densenet_169(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DenseNetEfficient(num_init_features=64, growth_rate=32, block_config=(6, 12, 32, 32),
                     **kwargs)
    if pretrained:
        model.load_state_dict(load_model('densenet-169.pth'))
    return model


def densenet_201(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DenseNetEfficient(num_init_features=64, growth_rate=32, block_config=(6, 12, 48, 32),
                     **kwargs)
    if pretrained:
        model.load_state_dict(load_model('densenet-201.pth'))
    return model


def densenet_161(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DenseNetEfficient(num_init_features=96, growth_rate=48, block_config=(6, 12, 36, 24),
                     **kwargs)
    if pretrained:
        model.load_state_dict(load_model('densenet-161.pth'))
    return model


def densenet_264_k32(pretrained=False, **kwargs):
    r"""Densenet-264 (k32) model from
    `"Memory-Efficient Implementation of DenseNets" <https://arxiv.org/abs/1707.06990>`_

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DenseNetEfficient(num_init_features=64, growth_rate=32, block_config=(6,12,64,48), cifar=False,
                              **kwargs)
    if pretrained:
        model.load_state_dict(load_model('densenet_264_k32.pth'))
    return model


def densenet_232_k48(pretrained=False, **kwargs):
    r"""Densenet-232 (k48) model from
    `"Memory-Efficient Implementation of DenseNets" <https://arxiv.org/abs/1707.06990>`_

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DenseNetEfficient(num_init_features=96, growth_rate=48, block_config=(6,12,48,48), cifar=False,
                              **kwargs)
    if pretrained:
        model.load_state_dict(load_model('densenet_232_k48.pth'))
    return model


def densenet_cosine_264_k32(pretrained=False, **kwargs):
    r"""Densenet Cosine-264 (k32) model from
    `"Memory-Efficient Implementation of DenseNets" <https://arxiv.org/abs/1707.06990>`_
    ...
    model = DenseNetEfficient(num_init_features=64, growth_rate=32, block_config=(6,12,64,48), cifar=False,
                              **kwargs)
    if pretrained:
        model.load_state_dict(load_model('densenet_cosine_264_k32.pth'))
    return model


def densenet_cosine_264_k48(pretrained=False, **kwargs):
    r"""Densenet Cosine-264 (k48) model from
    `"Memory-Efficient Implementation of DenseNets" <https://arxiv.org/abs/1707.06990>`_

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DenseNetEfficient(num_init_features=96, growth_rate=48, block_config=(6,12,64,48), cifar=False,
                              **kwargs)
    if pretrained:
        model.load_state_dict(load_model('densenet_cosine_264_k48.pth'))
    return model
