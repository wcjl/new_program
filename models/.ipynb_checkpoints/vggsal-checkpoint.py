...
def vggsal11(pretrained=True, init_weights=False):
...
def vggsal11_bn(pretrained=True, init_weights=False):
    main_net = vgg11_bn(pretrained).features
    readout_net = nn.Conv2d(512, 1, kernel_size=1, stride=1, padding=0)
    model = BaseModel(main_net, readout_net, single_fine_path=True)
    return model

def vggsal13(pretrained=True, init_weights=False):
    main_net = vgg13(pretrained).features
    readout_net = nn.Conv2d(512, 1, kernel_size=1, stride=1, padding=0)
    model = BaseModel(main_net, readout_net, single_fine_path=True)
    return model

def vggsal13_bn(pretrained=True, init_weights=False):
    main_net = vgg13_bn(pretrained).features
    readout_net = nn.Conv2d(512, 1, kernel_size=1, stride=1, padding=0)
    model = BaseModel(main_net, readout_net, single_fine_path=True)
    return model

def vggsal16(pretrained=True, init_weights=False):
    main_net = vgg16(pretrained).features
    readout_net = nn.Conv2d(512, 1, kernel_size=1, stride=1, padding=0)
    model = BaseModel(main_net, readout_net, single_fine_path=True)
    return model

def vggsal16_bn(pretrained=True, init_weights=False):
    main_net = vgg16_bn(pretrained).features
    readout_net = nn.Conv2d(512, 1, kernel_size=1, stride=1, padding=0)
    model = BaseModel(main_net, readout_net, single_fine_path=True)
    return model

def vggsal19(pretrained=True, init_weights=False):
    main_net = vgg19(pretrained).features
    readout_net = nn.Conv2d(512, 1, kernel_size=1, stride=1, padding=0)
    model = BaseModel(main_net, readout_net, single_fine_path=True)
    return model

def vggsal19_bn(pretrained=True, init_weights=False):
